...
class database{
	...
	public function __construct(){//本地调试使用注释中的值
		$this->mysql_host = 'localhost';//'localhost';//SAE_MYSQL_HOST_M;
		$this->mysql_db = 'hospital';//'hospital';//SAE_MYSQL_DB;
		$this->mysql_user = 'root';//'YOURUSERNAME';//SAE_MYSQL_USER;
		$this->mysql_password = 'qwe123';//'YOURPASSWORD';//SAE_MYSQL_PASS;
		$this->mysql_port = 3306;	//3306;//SAE_MYSQL_PORT;
		$this -> connect_to_db();
	}
	...
	public function get_field_from_table($table,$field,$keys){//表名，待查询字段（需要的结果），查询依据的字段及字段值（为关联数组形式）;返回值为关联数组形式
		...
		foreach($keys_index as $index){
			$str = $str."$index like '$keys[$index]' AND ";
		}
		$str = substr($str,0,-4);
